...
File.prototype = {
  ...
  load: function() {
    ...
    posix.cat(this.path, 'ascii').addCallback(function(data) {
      self.data = data;
      p.emitSuccess();
    });
    return p;
...
function Request(data, encoding, boundary) {
    ...
    this.eachPart(function(part) {
      part.encode(function(encoded) {
        ...
        if (sentCount == self.fieldCount) {
          request.sendBody(self.boundary, self.encoding);
          callback.call(self);
        }
      });
    });
...
process.mixin(exports, {
  ...
  encode: function(data, encoding, callback) {
    if (typeof encoding == 'function') {
      callback = encoding;
      encoding = 'ascii';
    }
    var r = new Request(data, encoding);
    r.encode(callback);
    
    return r;
  },
  send: function(request, data, encoding, callback) {
    if (typeof encoding == 'function') {
      callback = encoding;
      encoding = 'ascii';
    }
